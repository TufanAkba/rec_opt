...
def MoCa_3D(disc_z, r_1, L, dL):
    ...
    try:
        F_I_BP = np.load(str(disc_z)+str(r_1)+str(L)+'F_I_BP'+'.npy')
        F_I_1 = np.load(str(disc_z)+str(r_1)+str(L)+'F_I_1'+'.npy')
        return F_I_1,F_I_BP
    except IOError:
        ...
        np.save(str(disc_z)+str(r_1_temp)+str(L)+'F_I_BP',F_I_BP)
        np.save(str(disc_z)+str(r_1_temp)+str(L)+'F_I_1',F_I_1)
